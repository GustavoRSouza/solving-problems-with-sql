create schema bee2992;

CREATE TABLE  bee2992.departamento(
    cod_dep int,
    nome varchar(50),
    endereco varchar(50)
);

CREATE TABLE bee2992.dependente
(matr int, nome varchar(50), endereco varchar(50));

CREATE TABLE bee2992.desconto
(cod_desc int, nome varchar(50), tipo varchar(10), valor numeric);

CREATE TABLE bee2992.divisao
(cod_divisao int, nome varchar(50), endereco varchar(50), cod_dep int);

CREATE TABLE bee2992.emp_desc
(cod_desc int, matr int);

CREATE TABLE bee2992.emp_venc
(cod_venc int, matr int);

CREATE TABLE bee2992.empregado
...
CREATE TABLE bee2992.vencimento
(cod_venc int, nome varchar(50), tipo varchar(10), valor numeric);

INSERT INTO bee2992.departamento
...
INSERT INTO bee2992.dependente
(matr, nome, endereco)
VALUES
...
INSERT INTO bee2992.vencimento
...
/*
Listar as divisões com maiores médias salariais dentro de seus departamentos. 

A saída deverá apresentar o nome do departamento, 
o nome da divisão com maior média salarial do departamento e a média salarial da divisão. 

O resultado deve estar em ordem decrescente usando a média salarial.

Dica: Você pode utilizar a função COALESCE(check_expression , 0) para substituir algum valor 
null por zero; Além disso, você também pode utilizar a função ROUND(value, 2) para exibir os 
valores com 2 casas decimais.
*/
with vencimentos as (
	select
		a.matr,
		coalesce(sum(b.valor),0) as vencimento
	from
		bee2992.emp_venc a
	left join
		bee2992.vencimento b
...
descontos as (
	select
		a.matr,
		coalesce(sum(b.valor),0) as desconto
	from
		bee2992.emp_desc a
	left join
		bee2992.desconto b
...
sal_liquido as (
	select
		a.matr,
		coalesce(a.vencimento,0) - coalesce(b.desconto,0) as salario
	from
		vencimentos a
	left join
		descontos b
...
ranking_divisao as (
	select
		...
		b.nome as divisao,
		round(avg(d.salario),2) as media,
		rank() over (partition by a.nome order by avg(d.salario) desc) as ranking
	from
		bee2992.departamento a
	inner join
		bee2992.divisao b
